...
def init():
    ...
    #rknn.load_rknn('RetinaFace_mobile320-i8.rknn')
...
def letterbox( im, new_shape=(640, 640), color=(0, 0, 0)):
        ...
        padded_img = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border

        #padded_img = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
...
def get_faces(img):
    ...
    letterbox_img, aspect_ratio = letterbox_resize(img, (model_height,model_width), 114)  # letterbox缩放
    #letterbox_img, aspect_ratio = letterbox(img, (model_height,model_width), 114)
    ...
    boxes = boxes * scale // 1 
    boxes[...,0::2] =np.clip(boxes[...,0::2] / aspect_ratio, 0, img_width)  #letterbox
    boxes[...,1::2] =np.clip(boxes[...,1::2] / aspect_ratio, 0, img_height) #letterbox
    ...
    landmarks[...,0::2] = np.clip(landmarks[...,0::2] / aspect_ratio, 0, img_width) #letterbox
    landmarks[...,1::2] = np.clip(landmarks[...,1::2] / aspect_ratio, 0, img_height) #letterbox
